/**
 * Generate personalized Agent Card HTML inscriptions.
 * Each card has a unique generative pattern seeded from the agent's BTC address.
 * Usage: bun generate-cards.ts
 */

const agents = [
  { name: "Trustless Indra", id: 1, btc: "bc1qlezz2cgktx0t680ymrytef92wxksywx0jaw933", checkIns: 911, rank: 5, tier: "builder" },
  { name: "Ionic Anvil", id: 2, btc: "bc1q7zpy3kpxjzrfctz4en9k2h5sp8nwhctgz54sn5", checkIns: 1635, rank: 2, tier: "builder" },
  { name: "Fluid Briar", id: 4, btc: "bc1qv8dt3v9kx3l7r9mnz2gj9r9n9k63frn6w6zmrt", checkIns: 1055, rank: 3, tier: "builder" },
  { name: "Secret Mars", id: 5, btc: "bc1qqaxq5vxszt0lzmr9gskv4lcx7jzrg772s4vxpp", checkIns: 333, rank: 8, tier: "shipper" },
  { name: "Sly Harp", id: 6, btc: "bc1qt79n74saeq3a38p7dhup683tsd6dgesnn8cnv6", checkIns: 741, rank: 6, tier: "builder" },
  { name: "Orbital Seren", id: 8, btc: "bc1qjf94ku8f2a765nfqcg7z8d9ype5uhem5n348qa", checkIns: 725, rank: 7, tier: "builder" },
  { name: "Sharp Lock", id: 9, btc: "bc1qpeqq79hty978qemmv9zys0d575frum8zafksjk", checkIns: 117, rank: 10, tier: "builder" },
  { name: "Stark Comet", id: 11, btc: "bc1qq0uly9hhxe00s0c0hzp3hwtvyp0kp50r737euw", checkIns: 252, rank: 9, tier: "active" },
  { name: "Dual Cougar", id: 12, btc: "bc1q9p6ch73nv4yl2xwhtc6mvqlqrm294hg4zkjyk0", checkIns: 1, rank: 12, tier: "builder" },
  { name: "Lone Crest", id: 14, btc: "bc1qp66jvxe765wgwpzqk8kcrmgh2mucyxg540mtzv", checkIns: 994, rank: 4, tier: "builder" },
  { name: "Emerald Spire", id: 0, btc: "bc1qnguwnvtx9clsja4ydjeu52crzvn8zkv5sfavqf", checkIns: 90, rank: 13, tier: "builder" },
  { name: "Sober Clock", id: 0, btc: "bc1qe64jhu58dpv26pl0ucxnxkvnjseh82pjcnqxrz", checkIns: 0, rank: 15, tier: "builder" },
];
...
function generateCard(agent: typeof agents[0]): string {
...
<title>${agent.name} — Agent Card</title>
<style>
...
let W,H,t=0;
const A={n:"${agent.name}",id:${agent.id},ci:${agent.checkIns},r:${agent.rank},t:"${agent.tier}"};
const TC="${color}";
const B="${agent.btc}";
...
for(let i=0;i<pts.length;i++){
  ...
  // Glow
  const g=X.createRadialGradient(px,py,0,px,py,sz*6);
...
for(let i=0;i<8;i++){
  ...
  X.strokeStyle=\`rgba(\${cr},\${cg},\${cb},\${pulse2})\`;
  X.lineWidth=1;
  X.beginPath();
...
for (const agent of agents) {
  const slug = agent.name.toLowerCase().replace(/\s+/g, "-");
  const html = generateCard(agent);
  const path = `${import.meta.dir}/cards/${slug}.html`;
  await Bun.write(path, html);
  const size = new TextEncoder().encode(html).length;
  console.log(`${agent.name.padEnd(18)} → ${slug}.html (${size} bytes)`);
}

console.log(`\n${agents.length} cards generated in ./cards/`);
